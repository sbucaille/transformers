...
# taken from https://github.com/facebookresearch/rt_detr_v2/blob/master/models/matcher.py
...
# taken from https://github.com/facebookresearch/rt_detr_v2/blob/master/models/rt_detr_v2.py
class RTDetrV2Loss(nn.Module):
    """
    This class computes the losses for RTDetrV2ForObjectDetection/RTDetrV2ForSegmentation. The process happens in two steps: 1)
    we compute hungarian assignment between ground truth boxes and the outputs of the model 2) we supervise each pair
    of matched ground-truth / prediction (supervise class and box).

    A note on the `num_classes` argument (copied from original repo in rt_detr_v2.py): "the naming of the `num_classes`
    parameter of the criterion is somewhat misleading. It indeed corresponds to `max_obj_id` + 1, where `max_obj_id` is
    the maximum id for a class in your dataset. For example, COCO has a `max_obj_id` of 90, so we pass `num_classes` to
    be 91. As another example, for a dataset that has a single class with `id` 1, you should pass `num_classes` to be 2
    (`max_obj_id` + 1). For more details on this, check the following discussion
    https://github.com/facebookresearch/rt_detr_v2/issues/108#issuecomment-650269223"


    Args:
        matcher (`RTDetrV2HungarianMatcher`):
    ...
    def __init__(self, config, losses, weight_dict):
        ...
        self.matcher = RTDetrV2HungarianMatcher(
    ...
    def forward(self, outputs, targets):
        ...
        # In case of cdn auxiliary losses. For RTDetrV2
...
class RTDetrV2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, RTDetrV2Loss):
...
class RTDetrV2ForObjectDetectionCriterion(nn.Module):
    def __init__(self, config):
        super().__init__()
        requires_backends(self, ["scipy"])
        losses = ["vfl", "boxes"]
        ...
        self.loss_module = RTDetrV2Loss(config, losses, weight_dict)
