...
from ...loss.loss_for_object_detection import generalized_box_iou
...
from ..detr.modeling_detr import DetrHungarianMatcher, DetrLoss, _set_aux_loss
...
class GroundingDinoHungarianMatcher(DetrHungarianMatcher):
    @torch.no_grad()
...
class GroundingDinoLoss(DetrLoss):
...
class GroundingDinoCriterion(nn.Module):
    def __init__(self, config):
        ...
        self.auxiliary_loss = config.auxiliary_loss
        self.two_stage = config.two_stage
        self.loss_module = GroundingDinoLoss(config, losses, weight_dict)
    ...
    def forward(
        ...
        if self.two_stage:
            ...
            encoder_loss_dict = self.loss_module(encoder_outputs_loss, labels)
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: GroundingDinoConfig):
        ...
        self.criterion = GroundingDinoCriterion(config)
        self.post_init()
