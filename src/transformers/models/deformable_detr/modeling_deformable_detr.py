...
# taken from https://github.com/facebookresearch/deformable_detr/blob/master/models/matcher.py
...
# taken from https://github.com/facebookresearch/deformable_detr/blob/master/models/deformable_detr.py
class DeformableDetrLoss(nn.Module):
    """
    This class computes the losses for DeformableDetrForObjectDetection/DeformableDetrForSegmentation. The process happens in two steps: 1)
    we compute hungarian assignment between ground truth boxes and the outputs of the model 2) we supervise each pair
    of matched ground-truth / prediction (supervise class and box).

    A note on the `num_classes` argument (copied from original repo in deformable_detr.py): "the naming of the `num_classes`
    parameter of the criterion is somewhat misleading. It indeed corresponds to `max_obj_id` + 1, where `max_obj_id` is
    the maximum id for a class in your dataset. For example, COCO has a `max_obj_id` of 90, so we pass `num_classes` to
    be 91. As another example, for a dataset that has a single class with `id` 1, you should pass `num_classes` to be 2
    (`max_obj_id` + 1). For more details on this, check the following discussion
    https://github.com/facebookresearch/deformable_detr/issues/108#issuecomment-650269223"
...
class DeformableDetrForObjectDetectionCriterion(nn.Module):
    def __init__(self, config):
        super().__init__()
        requires_backends(self, ["scipy"])
        ...
        self.auxiliary_loss = config.auxiliary_loss
        self.loss_module = DeformableDetrLoss(config, losses, weight_dict)
